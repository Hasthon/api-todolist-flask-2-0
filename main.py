...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:vannia123@localhost:3306/todolistft9' 
db.init_app(app)
Migrate(app, db)
CORS(app)
manager = Manager(app)
...
def todos(username):
    ...
    if request.method == 'POST':
        ...
        #validar tipo de datos que estoy recibiendo sea una lista
        ...
        #convertir un array de objetos en string
    ...
    if request.method == 'PUT':
        #obtener el cuerpo completo del request
        body = request.get_json()
        #obtener cada una de las listas
        todo = Todo.query.filter_by(username=username).first()
        if todo:
            todo.tasks= json.dumps(body)
            todo.update()
